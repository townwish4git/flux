...
def unfuse_qkv(qkv, num_heads):
    ...
    q, k, v = ops.chunk(
        qkv.reshape(bsz, seq, 3, num_heads, -1)
        .transpose(2, 0, 3, 1, 4)
        .reshape(3 * bsz, num_heads, seq, -1),
        chunks=3,
        axis=0,
    )
    return q, k, v
...
class LastLayer(nn.Cell):
    def __init__(self, hidden_size: int, patch_size: int, out_channels: int):
        ...
        self.linear = nn.Dense(
            hidden_size, patch_size * patch_size * out_channels, has_bias=True
        )
        self.adaLN_modulation = nn.SequentialCell(
            nn.SiLU(), nn.Dense(hidden_size, 2 * hidden_size, has_bias=True)
        )
