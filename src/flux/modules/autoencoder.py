...
class AttnBlock(nn.Cell):
    def __init__(self, in_channels: int):
        ...
        self.q = nn.Conv2d(in_channels, in_channels, kernel_size=1, has_bias=True)
        self.k = nn.Conv2d(in_channels, in_channels, kernel_size=1, has_bias=True)
        self.v = nn.Conv2d(in_channels, in_channels, kernel_size=1, has_bias=True)
        self.proj_out = nn.Conv2d(
            in_channels, in_channels, kernel_size=1, has_bias=True
        )
    ...
    def attention(self, h_: Tensor) -> Tensor:
        ...
        h_ = ops.operations.nn_ops.FlashAttentionScore(
            head_num=1, input_layout="BNSD", scale_value=scale_factor
        )(
            q.to(ms.bfloat16),
            k.to(ms.bfloat16),
            v.to(ms.bfloat16),
...
class Downsample(nn.Cell):
    ...
    def construct(self, x: Tensor):
        pad = (0, 1, 0, 1)
        x = mint.functional.pad(x, pad, mode="constant", value=0)
        x = self.conv(x)
        return x
...
class Upsample(nn.Cell):
    ...
    def construct(self, x: Tensor):
        x = mint.functional.interpolate(x, scale_factor=2.0, mode="nearest")
        x = self.conv(x)
        return x
