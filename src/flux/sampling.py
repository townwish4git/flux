...
from .util import is_torch_available
...
def get_noise(
    ...
    if is_torch_available():
        ...
        noise = ms.Tensor.from_numpy(
            torch.randn(shape, generator=generator, dtype=_torch_dtype_map[dtype])
            .float()
            .numpy()
        ).to(
            dtype
        )  # use float32 as intermediary because numpy doesn't support bfloat16
    else:
        generator = np.random.default_rng(seed)
        noise = ms.Tensor.from_numpy(generator.standard_normal(size=shape), dtype=dtype)

    return noise


def prepare(
    t5: HFEmbedder, clip: HFEmbedder, img: Tensor, prompt: Union[str, List[str]]
) -> Dict[str, Tensor]:
    bs, c, h, w = img.shape
